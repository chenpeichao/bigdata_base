...
object AreaTop3Stat_Teacher {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("area").setMaster("local[*]").set("spark.ui.port", "8999")
    ...
    sparkSession.udf.register("group_concat_distinct", new GroupConcatDistinct_Teacher)
  ...
  def getTop3Product(sparkSession: SparkSession, taskUUID: String) = {
    ...
    val sql = "select area, " +
      ...
      "tmp_area_count_product_info) t where rank<=10"
  ...
  def getAreaProductClickCountInfo(sparkSession: SparkSession) = {
    ...
    val sql = "select tacc.area, tacc.city_infos, tacc.pid, pi.product_name, " +
      ...
      "tacc.click_count " +
      " from tmp_area_click_count tacc join commerce.product_info pi on tacc.pid = pi.product_id"
  ...
  def getCityAndProductInfo(sparkSession: SparkSession, taskParam: JSONObject) = {
    ...
    val sql = "select city_id, click_product_id from commerce.user_visit_action where date>='" + startDate +
